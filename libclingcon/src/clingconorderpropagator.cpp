...
//            std::cout << "Literal: " << p.rep() << "<-litnumber "<< p.var() << "," << p.sign() << " @"<< s_.decisionLevel() << std::endl;
            ...
            {   /// cspVar.first > bound
                ...
                //if (conf_.minLitsPerVar >= 0 || !conf_.explicitBinaryOrderClausesIfPossible) /// if i precreate all variables and do have explicitBinaryorderClauses, then i do not need to do this
                if (!conf_.explicitBinaryOrderClausesIfPossible || (conf_.minLitsPerVar>=0 && (uint64)(conf_.minLitsPerVar) < p_.getVVS().getVariableStorage().getDomain(cspVar.first).size()))
                {
...
void ClingconOrderPropagator::forceKnownLiteralGE(order::ViewIterator it, Clasp::Literal l)
{
    //assert(iterator is variterator)
    auto varit = order::ViewIterator::viewToVarIterator(it);
    order::pure_LELiteral_iterator pit(varit,p_.getVVS().getVariableStorage().getOrderStorage(varit.view().v),1);
    assert(pit.isValid());
    ++pit;
    assert(pit.isValid());
...
void ClingconOrderPropagator::forceKnownLiteralLE(order::ViewIterator it, Clasp::Literal l)
{
    auto varit = order::ViewIterator::viewToVarIterator(it);
    order::pure_LELiteral_iterator pit(varit,p_.getVVS().getVariableStorage().getOrderStorage(varit.view().v),0);
    assert(pit.isValid());
    --pit;
    assert(pit.isValid());
        ...
        if (clauses.size())
        {
            ...
            {               
                ...
                for (unsigned int i = 0; i < its.size(); ++i)
                {
                    ...
                    {
                        auto currentIt = p_.getVVS().getVariableStorage().getCurrentRestrictor(its[i].view());
                        Clasp::Literal l = toClaspFormat(vs.getVariableStorage().getGELiteral(its[i]));
...
//                              std::cout << std::endl;
                ...
                {
                    if (!Clasp::ClauseCreator::create(s_,claspClause, Clasp::ClauseCreator::clause_force_simplify, Clasp::ClauseCreator::ClauseInfo(Clasp::Constraint_t::Other)).ok())
                        return false;
                }
                else
                {
...
{/*
    for (order::Variable var = 0; var < p_.getVVS().getVariableStorage().numVariables(); ++var)
    {
        if (watched_[var] && p_.getVVS().getVariableStorage().isValid(var))
        {
            ...
            order::pure_LELiteral_iterator it(baselr.begin(),p_.getVVS().getVariableStorage().getOrderStorage(var),true);
            ...
            while(it.isValid())
            {
                ...
                if (s_.isFalse(toClaspFormat(*it)) && it.numElement() >= currentlr.begin().numElement())
                    assert(false);
                if (s_.isTrue(toClaspFormat(*it)) && it.numElement() < currentlr.end().numElement()-1)
                    assert(false);
                if (!s_.isFalse(toClaspFormat(*it)) && !s_.isTrue(toClaspFormat(*it)) && (it.numElement() < currentlr.begin().numElement() || it.numElement() >= currentlr.end().numElement()))
                {
//                    std::cout << "checking literal " << (*it).asUint() << " mit elementindex " << it.numElement() << std::endl;
//                    std::cout << "this is v" << var << "<=" << *(baselr.begin()+it.numElement()) << std::endl;
//                    std::cout << "Orig domain " << baselr.lower() << ".." << baselr.upper() << " is "
//                              << currentlr.lower() << ".." << currentlr.upper() << std::endl;
                    assert(false);
                }
    ...
    else
    {
        ...
        //for (order::Variable v = 0; v < p_.getVVS().getVariableStorage().numVariables(); ++v)
        {
            ...
            else /// not watched, need to search for value
            {
                ...
                vit = my_upper_bound(rs.begin(), rs.end(),s_,p_.getVVS().getVariableStorage().getOrderStorage(var));
            }
            int32 value = (((int64)(e.secondCoef) * (int64)(*vit)) + (int64)(e.constant))/(int64)(e.firstCoef);
            assert((((int64)(e.secondCoef) * (int64)(*vit)) + (int64)(e.constant)) % (int64)(e.firstCoef) == 0);
            lastModel_[v]=value;
        }
