...
ViewIterator lower_bound(ViewIterator first, ViewIterator last, int64 value)
{
    ...
    int64 u = view.multiply( view.reversed() ? first.it_.d_->getRanges()[(first.it_-1).index_].l : first.it_.d_->getRanges()[first.it_.index_].u);
    if (value > u)
    while (count > 0) {
        /// if in current range (value<=u)

        it = first;
        int64 changer = view.reversed() ? (first.it_-1).steps_ : first.it_.d_->getRanges()[first.it_.index_].size()-first.it_.steps_-1;
...
ViewIterator upper_bound(ViewIterator first, ViewIterator last, int64 value)
{
    ...
    int64 diff = std::ceil(double(value - (*first))/std::abs(view.a)) + 1;
    if (diff>0)
        first +=diff;
    return first;*/
}
