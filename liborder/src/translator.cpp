...
public:
    ...
    bool recTrans(int64 current, std::size_t index)
    {
        ...
        //auto i = order::wrap_lower_bound(lr.begin(),lr.end(),c.getRhs(),[&](int64 p, int64 val)
        //{ return p + current + subsums[index+1].second <= val; } );
        auto i = order::wrap_upper_bound(lr.begin(),lr.end(),int64(c.getRhs())-subsums[index+1].second - current);
                //{ return p + current + subsums[index+1].second <= val; } );
        while(i != lr.end())
        {
