...
{
    assert(order::upper_bound(first,last,value)==std::upper_bound(first,last,value));
    return order::upper_bound(first,last,value);
}
...
inline ViewIterator wrap_lower_bound(ViewIterator first, ViewIterator last, int64 value)
{
    ...
    return order::lower_bound(first,last,value);
}
